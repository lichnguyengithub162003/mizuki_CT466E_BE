...
test('it defines customer registration validation rules', function (): void {
    ...
    expect($rules)->toHaveKeys(['name', 'email', 'phone', 'password'])
        ...
        ->and($rules['phone'])->toContain('required', 'string', 'regex:/^0[35789][0-9]{8}$/')
        ...
        ->and(collect($rules['phone'])->contains(fn (mixed $rule): bool => $rule instanceof Unique))->toBeTrue()
...
test('it provides Vietnamese registration validation messages', function (): void {
    ...
    expect($messages['name.required'])->toBe('Vui lòng nhập họ tên')
        ...
        ->and($messages['phone.unique'])->toBe('Số điện thoại này đã được sử dụng')
